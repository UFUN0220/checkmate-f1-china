...
| `waitingDaysReported`     | internal          | 来源 `Waiting Day(s)`；只做审计对照，不覆盖固定快照截止日计算出的 Pending duration。                             |
| `sourceMonth`             | normalized/public | 来源月份页面，例如 `2026-01`；不是日期真值。                                                                     |
| `fetchedAt`               | internal/manifest | 页面获取时间；不作为 Check 月份。                                                                                |
| `snapshotDate`            | public/manifest   | 本次数据快照日；Pending 案例统一使用该日期作为 effective end date。                                              |
...
```text
pendingDurationDays = DATA_CUTOFF_DATE - checkDate
resolvedDurationDays = completeDate - checkDate
```

- `pendingAgeDays` 只用于 `status = pending`，本版固定使用 `snapshotDate - checkDate`，不是预计完成时间；来源 `Waiting Day(s)` 只作为质量审计字段。
- `effectiveEndDate` 对 Pending 固定为 `snapshotDate`；对已结束记录使用原始合法 `completeDate`。
- `durationDays` 是所有有合法 effective end date 的案例统一使用的 calendar-day duration；同一天为 0 天。
