...
- `DataQualityReport`：候选数、纳入/排除、异常旗标、重复、月份冲突、等待日冲突和敏感字段扫描。

## 规则

- F-1 仅接受 `f1`、`f-1`、`f 1`；地点仅接受北京、上海、广州、沈阳、武汉及代码内显式别名。
- 状态只接受 Pending、Clear、Reject；未知状态、未知地点、非 F-1、非法日期和无法判断的日期顺序不进入公开快照。
- Pending age 使用 `snapshotDate - checkDate`；Clear duration 使用 `completeDate - checkDate`；Reject 不参与完成时长。
...
`DemoFixtureAdapter` 是当前默认入口；`CheckeeExportAdapter` 接受开发者提供的 CSV/JSON；`CheckeeHtmlAdapter` 目前只执行 fail-closed 检查并抛出明确错误。默认设置为：
...
当前快照为 `DEMO_DATA` 合成数据，仅用于开发和截图。它不代表 Checkee 真实样本、总体概率、官方处理速度或个案预测。获得授权后，适配器应输出同一 `NormalizedCase`，不需要改写页面或统计层。
